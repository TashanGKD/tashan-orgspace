...
OrgSpace 当前及 v1 产品范围不再包含面向用户的通用计算执行或网站托管能力。

这不是删除历史讨论，而是明确延期。相关历史设计保留为决策记录，但不再作为当前产品承诺、实施阶段、导航入口、CLI 命令、API capability 或验收目标。
...
### 方案 C：继续留在导航并标记“即将上线”（未选择）

优点：改动最少。缺点：仍形成产品承诺，用户会把延期能力理解为正在开发。
...
`personal.usage` 若保留，只展示存储额度与文件用量，不展示 CPU、内存或计算资源。

前端蓝图删除运行、构建、服务、数据库和域名界面，不再以 `coming_soon` 展示。
...
- 门禁拒绝延期模块 ID 再次进入 `product-modules.json`；
- 门禁拒绝 `runtime.*`、`build.*`、`service.*`、`database.*` capability 进入当前注册表；
- 门禁检查 README、总设计、前端蓝图和交付计划不再把延期能力列为当前或 v1 范围；
- self-test 分别注入一个运行模块、一个服务 capability 和一条当前范围承诺，确认三者都会阻断 CI。
