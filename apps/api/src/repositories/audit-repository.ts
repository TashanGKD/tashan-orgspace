  ...
  public async append(transaction: TransactionClient, input: AppendAuditInput) {
    const [event] = await transaction<{ id: string }[]>`
      insert into audit_events (
        id, organization_id, account_id, principal_id, session_id, device_id,
        server_ip, proxy_chain, device_metadata, actor_source, reported_actor_source,
        ...
        ${input.id ?? transaction`gen_random_uuid()`},
        ...
        ${input.actorSource}, ${input.reportedActorSource ?? null},
        ...
        ${input.prevHash ?? null}, ${input.eventHash}, ${input.chainPosition},
        ${input.createdAt ?? transaction`now()`}
      )
      returning id
    `;
